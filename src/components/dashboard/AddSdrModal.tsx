    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogTrigger asChild>
        <Button size="sm" variant="outline"
          className="w-full justify-start border-[#333] text-gray-400 hover:text-white hover:border-gray-500 text-xs tracking-widest uppercase">
          <ClipboardList className="w-3 h-3 mr-1" /> Registrar SDR
        </Button>
      </DialogTrigger>

      <DialogContent className="bg-[#1A1A1A] border-[#333] text-white max-w-sm max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-gray-300 font-black tracking-widest uppercase text-sm">
            Registrar SDR
          </DialogTitle>
        </DialogHeader>
          ...
          {selected && !saved && (
            <motion.div key="status" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}
              ...
              <div className="bg-[#252525] border border-[#333] rounded-xl px-3 py-2.5 flex items-center justify-between">
                <div>
                  <p className="text-gray-300 font-black text-sm">{selected.cliente}</p>
              ...
              <Button onClick={handleSave} disabled={!status || saving}
                className="w-full bg-gray-400 text-black font-black uppercase tracking-widest hover:bg-gray-300 disabled:opacity-30">
